...
function keccak256(x1, x2, x3, x4, y1, y2, y3, y4) -> z1, z2, z3, z4 {
	let input:i32 := to_internal_i32ptr(x1, x2, x3, x4)
	let len:i32 := u256_to_i32(y1, y2, y3, y4)
    ...
    //	c->pt = j;
	let j:i32 := pt
	for { let i:i32 := 0:i32 } i32.lt_u(i, len) { i := i32.add(i, 1:i32) }
	{
		let stb_j:i32 := i32.add(j, i32.add(0x100:i32, slot))
		j := i32.add(j, 1:i32)
		i8_store(stb_j, i64.and(i64.xor(i8_load(stb_j), i8_load(i32.add(input, i))), 0xff))
		if i32.ge_u(j, rsiz)
		{
			keccakf(slot)
			j := 0:i32

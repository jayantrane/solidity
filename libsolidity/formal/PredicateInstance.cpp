...
{
	auto& state = _context.state();
	vector<smtutil::Expression> stateExprs{state.thisAddress(), state.state()};
...
smtutil::Expression constructor(Predicate const& _pred, ContractDefinition const& _contract, EncodingContext& _context)
{
	...
	auto& state = _context.state();
	vector<smtutil::Expression> stateExprs{state.errorFlag().currentValue(), state.thisAddress(), state.state()};
	return _pred(stateExprs + currentStateVariables(_contract, _context));
}
...
smtutil::Expression functionBlock(Predicate const& _pred, FunctionDefinition const& _function, ContractDefinition const* _contract, EncodingContext& _context)
{
...
vector<smtutil::Expression> currentFunctionVariables(FunctionDefinition const& _function, ContractDefinition const* _contract, EncodingContext& _context)
{
	auto& state = _context.state();
	vector<smtutil::Expression> exprs{_context.state().errorFlag().currentValue(), state.thisAddress(), state.state(0)};

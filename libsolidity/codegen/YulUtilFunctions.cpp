...
)
{
	...
	{
		s256 signedValue = u2s(baseValue);
		...
		exponentUpperbound = findExponentUpperbound(abs(signedValue), abs(_commonType.minValue()));

		bigint power = boost::multiprecision::pow(bigint(signedValue), exponentUpperbound);
		bigint overflowedPower = boost::multiprecision::pow(bigint(signedValue), exponentUpperbound + 1);
	...
	else
	{
		...
		solAssert(
			boost::multiprecision::pow(bigint(baseValue), exponentUpperbound) <= _commonType.maxValue() &&
			boost::multiprecision::pow(bigint(baseValue), exponentUpperbound + 1) > _commonType.maxValue(),
			"Incorrect exponent upper bound calculated."
		);
	}
